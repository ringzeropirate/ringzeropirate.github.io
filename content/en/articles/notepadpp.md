...
10. [CVE-2026-52884: Bypass via Path Traversal and LOLBin](#cve-003)
11. [Responsible Disclosure Timeline](#disclosure)
...
The fix covered the most straightforward injection scenarios but was bypassable via **Living Off The Land Binaries (LOLBins)** and a **path traversal** in the trusted directory check itself. Both bypass vectors are fully documented and addressed in section 10 (CVE-2026-52884).
...
The `isInTrustedDirectory()` function introduced in v8.9.6.1 (RunDlg.cpp) validates the executable path against a whitelist of trusted directories before calling `ShellExecute`. However, the check uses a prefix-based comparison (`PathIsPrefix()`) **without canonicalizing the path first**. This allows two distinct bypass vectors.

### Root Cause

```cpp
// VULNERABLE — RunDlg.cpp (v8.9.6.1)
bool isInTrustedDirectory(const wchar_t* path) {
    ...
    for (auto& trusted : trustedDirs) {
        if (PathIsPrefix(trusted, path))  // BUG: no canonicalization
            return true;
    }
    return false;
}
...
A `..\..\` sequence after a trusted prefix passes `PathIsPrefix()` while resolving to an untrusted location:

| Command in shortcuts.xml | Check result | Warning shown | Outcome |
|--------------------------|-------------|---------------|---------|
| `C:\Users\victim\malware.exe` | Untrusted | ✅ Yes | Popup shown |
| `C:\Windows\System32\..\..\Users\victim\malware.exe` | Trusted prefix match | ❌ No | **Executes silently** |
| `C:\Program Files\..\..\Users\victim\payload.exe` | Trusted prefix match | ❌ No | **Executes silently** |

### Bypass 2 — LOLBin via Trusted Launcher (CWE-59)

Binaries in trusted System32 pass the check while executing arbitrary code through their arguments:

| Command | Resolved binary | Warning | Outcome |
|---------|----------------|---------|---------|
| `cmd.exe /c malware.exe` | `C:\Windows\System32\cmd.exe` | ❌ No | **Executes silently** |
| `powershell.exe -enc BASE64` | `C:\Windows\System32\powershell.exe` | ❌ No | **Executes silently** |
| `rundll32.exe javascript:...` | `C:\Windows\System32\rundll32.exe` | ❌ No | **Executes silently** |
...
shortcuts.xml
  <Command>C:\Windows\System32\..\..\Users\victim\malware.exe</Command>
      │
      ▼ Parameters.cpp:3658
  NppXml::value(aNode)  ← SOURCE (same as CVE-2026-48800)
      │
      ▼ NppCommands.cpp:4264
  Command cmd(string2wstring(ucmd.getCmd(), CP_UTF8))
      │
      ▼ RunDlg.cpp
  isInTrustedDirectory(cmd2Exec)  ← PREFIX CHECK WITHOUT CANONICALIZATION
      │  PathIsPrefix("C:\\Windows\\System32\\",
      │    "C:\\Windows\\System32\\..\\..\\Users\\victim\\malware.exe")
      │  → returns TRUE (bypass!)
      ▼ RunDlg.cpp:221
  ShellExecute(hWnd, "open", cmd2Exec, ...)  ← SINK, no warning
```

### The Fix (commit ea15088)

```cpp
// FIXED — RunDlg.cpp (v8.9.6.2)
bool isInTrustedDirectory(const wchar_t* path) {
    wchar_t canonicalPath[MAX_PATH] = {};
    // Resolve all .., . and redundant separators FIRST
...
After canonicalization, `C:\Windows\System32\..\..\Users\victim\malware.exe` resolves to `C:\Users\victim\malware.exe` — which fails the trusted directory check and correctly triggers the warning popup.

The LOLBin bypass is addressed by the HMAC integrity check on shortcuts.xml (introduced alongside the trusted directory check): any externally injected command — regardless of the executable path — triggers a tampering warning at execution time.
...
| Day 9 | Don Ho releases Notepad++ v8.9.6.2 
| Day 18| GitHub assigns CVE-2026-52884 - advisories published publicly on GitHub Security 
| Day 20| Public Disclosure
