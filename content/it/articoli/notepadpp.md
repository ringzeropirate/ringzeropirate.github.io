...
description: "Ricerca di sicurezza indipendente che individua 3 CVE in *Notepad++ v8.9.5 — Risolto in v8.9.6.1 / v8.9.6.2*"
draft: false
---

**Autore:** Michele Piccinni  
**Tempo di lettura:** 10-14 minuti
...
10. [CVE-2026-52884: Bypass via Path Traversal e LOLBin](#cve-003)
11. [Timeline di Responsible Disclosure](#disclosure)
...
**Copertura in v8.9.6.1:**

| Scenario | Risolto? |
|----------|----------|
| `C:\Users\evil\malware.exe` | ⚠️ Popup di avviso mostrato — l'utente può comunque procedere |
| `calc.exe` (System32) | ⚠️ Consentito silenziosamente — ma visivamente ovvio per l'utente |
...
La fix copriva gli scenari di injection diretta più comuni ma era aggirabile tramite **Living Off The Land Binaries (LOLBin)** e un **path traversal** nel controllo stesso delle directory trusted. Entrambi i vettori di bypass sono documentati e risolti nella sezione 10 (CVE-2026-52884).

---

## 10. CVE-2026-52884: Bypass via Path Traversal e LOLBin {#cve-003}

**Advisory:** [GHSA-p58x-r3c9-x9p6](https://github.com/notepad-plus-plus/notepad-plus-plus/security/advisories/GHSA-p58x-r3c9-x9p6)  
**Versione affetta:** Notepad++ v8.9.6.1  
**Versione patchata:** v8.9.6.2  
**CVSS:** `CVSS:3.1/AV:L/AC:L/PR:N/UI:R/S:U/C:H/I:H/A:H` = **7.8 HIGH**  
**CWE:** CWE-42 (Path Traversal) / CWE-59 (Improper Link Resolution)  
**Credits:** Michele Piccinni, Trung Nguyen, Noman Nasir Minhas, Vibhum Dubey

### Descrizione

La funzione `isInTrustedDirectory()` introdotta in v8.9.6.1 (RunDlg.cpp) valida il percorso dell'eseguibile contro una whitelist di directory trusted prima di chiamare `ShellExecute`. Tuttavia, il controllo usa un confronto prefisso (`PathIsPrefix()`) **senza canonicalizzare il path prima**, permettendo due distinti vettori di bypass.

### Causa Radice

```cpp
// VULNERABILE — RunDlg.cpp (v8.9.6.1)
bool isInTrustedDirectory(const wchar_t* path) {
    const wchar_t* trustedDirs[] = {
        L"C:\\Program Files\\",
    ...
    for (auto& trusted : trustedDirs) {
        if (PathIsPrefix(trusted, path))  // BUG: nessuna canonicalizzazione
            return true;
    }
    return false;
}
...
### Bypass 1 — Path Traversal (CWE-42)

Una sequenza `..\..\` dopo un prefisso trusted supera `PathIsPrefix()` risolvendo comunque in una posizione non trusted:

| Comando in shortcuts.xml | Risultato check | Popup | Esito |
|--------------------------|----------------|-------|-------|
| `C:\Users\victim\malware.exe` | Non trusted | ✅ Si | Popup mostrato |
| `C:\Windows\System32\..\..\Users\victim\malware.exe` | Prefisso trusted | ❌ No | **Eseguito silenziosamente** |
| `C:\Program Files\..\..\Users\victim\payload.exe` | Prefisso trusted | ❌ No | **Eseguito silenziosamente** |

### Bypass 2 — LOLBin via Launcher Trusted (CWE-59)

I binari in System32 superano il check eseguendo codice arbitrario tramite i propri argomenti:

| Comando | Binario risolto | Popup | Esito |
|---------|----------------|-------|-------|
| `cmd.exe /c malware.exe` | `C:\Windows\System32\cmd.exe` | ❌ No | **Eseguito silenziosamente** |
| `powershell.exe -enc BASE64` | `C:\Windows\System32\powershell.exe` | ❌ No | **Eseguito silenziosamente** |
| `rundll32.exe javascript:...` | `C:\Windows\System32\rundll32.exe` | ❌ No | **Eseguito silenziosamente** |

### Flusso di Taint

```
shortcuts.xml
  <Command>C:\Windows\System32\..\..\Users\victim\malware.exe</Command>
      │
      ▼ Parameters.cpp:3658
  NppXml::value(aNode)  ← SOURCE (identico a CVE-2026-48800)
      │
      ▼ NppCommands.cpp:4264
  Command cmd(string2wstring(ucmd.getCmd(), CP_UTF8))
      │
      ▼ RunDlg.cpp
  isInTrustedDirectory(cmd2Exec)  ← CHECK PREFISSO SENZA CANONICALIZZAZIONE
      │  PathIsPrefix("C:\\Windows\\System32\\",
      │    "C:\\Windows\\System32\\..\\..\\Users\\victim\\malware.exe")
      │  → restituisce TRUE (bypass!)
      ▼ RunDlg.cpp:221
  ShellExecute(hWnd, "open", cmd2Exec, ...)  ← SINK, nessun avviso
```

### La Fix (commit ea15088)

```cpp
// FIXATO — RunDlg.cpp (v8.9.6.2)
bool isInTrustedDirectory(const wchar_t* path) {
    wchar_t canonicalPath[MAX_PATH] = {};
    // Risolve tutti .., . e separatori ridondanti PRIMA
    if (!PathCanonicalize(canonicalPath, path))
        return false;
    // ORA controlla contro le directory trusted
    const wchar_t* trustedDirs[] = {
        L"C:\\Program Files\\",
        L"C:\\Program Files (x86)\\",
        L"C:\\Windows\\System32\\",
...
Dopo la canonicalizzazione, `C:\Windows\System32\..\..\Users\victim\malware.exe` si risolve in `C:\Users\victim\malware.exe` — che fallisce il controllo trusted e mostra correttamente il popup di avviso.

Il bypass LOLBin e coperto dal controllo di integrita HMAC su shortcuts.xml: qualsiasi comando iniettato esternamente — indipendentemente dal percorso dell'eseguibile — attiva un avviso di tampering al momento dell'esecuzione.
...
| Giorno 9 | Don Ho rilascia Notepad++ v8.9.6.2
| Giorno 18| GitHub assegna CVE-2026-52884 advisory pubblicato pubblicamente su GitHub Security
| Giorno 20| Public Disclosure
...
*CVE-2026-52884: [GHSA-p58x-r3c9-x9p6](https://github.com/notepad-plus-plus/notepad-plus-plus/security/advisories/GHSA-p58x-r3c9-x9p6)*
